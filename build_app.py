...
:root {
  ...
  --cat-2: #d9722e;
  --cat-3: #1f8f5f;
  --shadow: 0 1px 2px rgba(28,24,12,0.06), 0 4px 16px rgba(28,24,12,0.05);
  --radius: 10px;
}
...
function renderShell(){
  ...
  document.getElementById('sidebar-foot').innerHTML = 'Henrique &amp; Carol<br>Orçamento familiar compartilhado<br><a href="/logout" style="color:inherit">Sair</a>';
...
function renderLancamentos(){
  ...
  var rows = txs.length ? txs.map(function(t){
    return '<tr>' +
      ...
      '<td>' + escapeHtml(t.descricao) +
        (t.parcela ? ' <span class="mono" style="font-size:11.5px;color:var(--text-muted);background:var(--surface-2);border-radius:5px;padding:1px 6px">' + escapeHtml(t.parcela) + '</span>' : '') +
        '<div style="font-size:11.5px;color:var(--text-muted)">' + escapeHtml(t.categoria) + (t.subcategoria ? ' · ' + escapeHtml(t.subcategoria) : '') + '</div></td>' +
...
function renderAjustes(){
  document.getElementById('app').innerHTML =
    ...
    '<div class="card" style="max-width:480px;margin-bottom:16px"><div class="section-title">Renda mensal</div>' +
      ...
      '</div>' +
    '</div>' +
    '<div class="card" style="max-width:480px;margin-bottom:16px"><div class="section-title">Aparência</div>' +
      ...
      '</select></div>' +
    '</div>' +
    '<div class="card" style="max-width:480px"><div class="section-title">Conta</div>' +
...
function init(){
  ...
  fetch(API_STATE_URL).then(function(r){
    ...
    if (e && e.code === 'auth') { location.href = '/login'; return; }
...
<div id="shell">
  <nav id="sidebar">
    <div class="brand"><img class="brand-mark" src="/logo.png" alt="" width="34" height="34">Orçamento</div>
...
LOGO_SRC = os.path.join(OUT_DIR, 'assets', 'cowhand-logo.png')
LOGO_DEST = os.path.join(PUBLIC_DIR, 'logo.png')
shutil.copyfile(LOGO_SRC, LOGO_DEST)
print("Copied", LOGO_SRC, "->", LOGO_DEST)
